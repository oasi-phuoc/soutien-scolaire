  ...
  return (
    <section className="rounded-[var(--radius-lg)] border border-[var(--color-theme)]/20 bg-[var(--color-theme-light)] p-4">
      <div className="mb-3 flex items-center gap-2">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-[var(--color-theme)]" aria-hidden>
          <path d="M9 11l3 3L22 4" />
          <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" />
        </svg>
        <h2 className="text-sm font-semibold text-[var(--color-theme)]">
          Tâches{" "}
          {pending.length > 0 && (
            <span className="ml-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[var(--color-theme)] px-1.5 text-[10px] font-bold text-white">
              {pending.length}
            </span>
          )}
        </h2>
            ...
            return (
              <li key={a.assignment_id} className="flex items-start gap-3 rounded-xl bg-white px-3 py-2.5 shadow-sm dark:bg-zinc-900">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="shrink-0 rounded-full bg-[var(--color-theme)]/15 px-3 py-1.5 text-xs font-semibold text-[var(--color-theme)] transition-colors hover:bg-[var(--color-theme)]/25 disabled:opacity-50"
                >
                  Fait ✓
                </button>
              </li>
