...
export function StudentPersonalInfoCard({
  ...
  const parsed = parseEleveClasse(student.classe);
  ...
  function saveClasse() {
    ...
    if (next === student.classe) return;
    startTransition(async () => {
      const res = await setStudentClasseAction(student.id, next);
      if (!res.ok) {
        setError(res.reason ?? "Impossible de changer la classe.");
        return;
      }
      onClasseChange?.(student.id, res.classe ?? next);
    });
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-950 ${pending ? "opacity-80" : ""}`}>
        <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-zinc-400">Classe</p>
