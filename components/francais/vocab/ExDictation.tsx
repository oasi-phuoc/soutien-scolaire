          ...
          return (
            ...
            >
              ...
              {s.checked && !s.correct && revealCorrection ? (
                <div className="h-8 flex w-full flex-col justify-center rounded-none border-0 border-b-2 border-amber-500 px-0">
                  <span className="text-[10px] leading-none text-[var(--color-text-secondary)]">{s.answer || "—"}</span>
                  <span className="text-xs font-bold leading-none text-amber-600">{w.word}</span>
                </div>
              ) : (
                <input
                  type="text"
