import {
  readingPoolExercise,
  type CommunicationExercise,
  type ExpressPoDialogue,
  type ExpressPePrompt,
} from "./express-types";
import { buildExpressPool, type ExpressRawQ } from "./express-listening-helpers";

function q(item: ExpressRawQ): ExpressRawQ {
  return item;
}

/* ── Compréhension écrite — E3.2 La vie quotidienne ── */

const CE_TEXT_1 = `SMS après le travail

Le sujet principal est les courses. ma mère donne l'information.
Le moment indiqué est à 17 h. Le rendez-vous est à la Migros du centre.
Consigne : venir à pied. Détail : le budget est de 45 francs.
Les horaires habituels restent les mêmes.
Une question ? Écrivez ou téléphonez.
Merci encore, et à bientôt.`;

const CE_POOL_1 = buildExpressPool("e3-2-1", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un SMS", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un SMS",
    vfQ: "Le texte mentionne un SMS.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["les courses", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "les courses",
    vfQ: "Le texte mentionne les courses.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 17 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 17 h",
    vfQ: "Le texte mentionne à 17 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la Migros du centre", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la Migros du centre",
    vfQ: "Le texte mentionne à la Migros du centre.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["ma mère", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "ma mère",
    vfQ: "Le texte mentionne ma mère.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_2 = `Agenda personnel

Petit message pour un rendez-vous médical.
le médecin propose un rendez-vous au cabinet du Dr Martin à 10 h 30.
Action demandée : prendre le bus 5.
la carte d'assurance est dans le sac.
Apportez une pièce d'identité si possible.
Le lieu est accessible en bus et à pied.
Un plan est disponible sur demande.
Nous vous attendons avec plaisir.
Les places sont limitées, merci de confirmer.
Merci de lire ce message jusqu'à la fin.`;

const CE_POOL_2 = buildExpressPool("e3-2-2", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un agenda", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un agenda",
    vfQ: "Le texte mentionne un agenda.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un rendez-vous médical", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un rendez-vous médical",
    vfQ: "Le texte mentionne un rendez-vous médical.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 10 h 30", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 10 h 30",
    vfQ: "Le texte mentionne à 10 h 30.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au cabinet du Dr Martin", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au cabinet du Dr Martin",
    vfQ: "Le texte mentionne au cabinet du Dr Martin.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["le médecin", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "le médecin",
    vfQ: "Le texte mentionne le médecin.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le bus 5", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le bus 5",
    vfQ: "Le texte mentionne prendre le bus 5.",
    vfC: 0,
  }),
]);
const CE_TEXT_3 = `WhatsApp famille

À noter : un déjeuner.
On voit mes parents au restaurant Le Lac. Le moment choisi est à 12 h 30.
Après cela, action simple : venir en voiture.
Rappel : Détail : la table est près de la fenêtre.
Tu peux m'appeler si c'est plus simple pour toi.
J'espère que tu vas bien et que tout se passe comme prévu.
N'oublie pas de me confirmer dès que tu peux.
Sinon on peut aussi en parler demain matin.
Merci de votre attention et de votre patience.`;

const CE_POOL_3 = buildExpressPool("e3-2-3", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un WhatsApp", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un WhatsApp",
    vfQ: "Le texte mentionne un WhatsApp.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un déjeuner", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un déjeuner",
    vfQ: "Le texte mentionne un déjeuner.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 12 h 30", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 12 h 30",
    vfQ: "Le texte mentionne à 12 h 30.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au restaurant Le Lac", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au restaurant Le Lac",
    vfQ: "Le texte mentionne au restaurant Le Lac.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["mes parents", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "mes parents",
    vfQ: "Le texte mentionne mes parents.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir en voiture", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir en voiture",
    vfQ: "Le texte mentionne venir en voiture.",
    vfC: 0,
  }),
]);
const CE_TEXT_4 = `Note sur la porte

Information courte. Tom parle de le sport.
Le lieu change ou se confirme : chez FitPlus.
Le moment est à 19 h. La consigne reste claire : venir à vélo.
le badge est dans la poche.
Une réponse rapide nous aide beaucoup.
Vous pouvez venir avec un ami ou un membre de la famille.
Apportez une pièce d'identité si possible.
Merci encore pour votre compréhension.`;

const CE_POOL_4 = buildExpressPool("e3-2-4", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["une note", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "une note",
    vfQ: "Le texte mentionne une note.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["le sport", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "le sport",
    vfQ: "Le texte mentionne le sport.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 19 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 19 h",
    vfQ: "Le texte mentionne à 19 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["chez FitPlus", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "chez FitPlus",
    vfQ: "Le texte mentionne chez FitPlus.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Tom", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Tom",
    vfQ: "Le texte mentionne Tom.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à vélo", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à vélo",
    vfQ: "Le texte mentionne venir à vélo.",
    vfC: 0,
  }),
]);
const CE_TEXT_5 = `Notification bancaire

Aujourd'hui, on organise un retrait.
Départ ou début à 14 h. Le groupe arrive à la banque UBS.
le conseiller explique la suite. Pendant l'activité : venir à pied.
le retrait prévu est de 200 francs.
Ce document complète les informations déjà données.
Nous comptons sur vous.
Voici quelques détails utiles pour la suite.
Je reste près de mon téléphone aujourd'hui.`;

const CE_POOL_5 = buildExpressPool("e3-2-5", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["une notification", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "une notification",
    vfQ: "Le texte mentionne une notification.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un retrait", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un retrait",
    vfQ: "Le texte mentionne un retrait.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 14 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 14 h",
    vfQ: "Le texte mentionne à 14 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la banque UBS", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la banque UBS",
    vfQ: "Le texte mentionne à la banque UBS.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["le conseiller", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "le conseiller",
    vfQ: "Le texte mentionne le conseiller.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_6 = `Message du coiffeur

Bonjour. Le dossier concerne une coupe de cheveux.
Il est possible de voir Clara à 16 h.
Le point de rendez-vous est au salon Élégance.
Pour continuer, action nécessaire : prendre le tram 12. Détail : arriver cinq minutes avant.
N'oubliez pas de vérifier la date.
Nous vous souhaitons une excellente journée.
Bonne journée à toutes et à tous.
Ce document complète les informations déjà données.`;

const CE_POOL_6 = buildExpressPool("e3-2-6", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un message", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un message",
    vfQ: "Le texte mentionne un message.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["une coupe de cheveux", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "une coupe de cheveux",
    vfQ: "Le texte mentionne une coupe de cheveux.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 16 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 16 h",
    vfQ: "Le texte mentionne à 16 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au salon Élégance", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au salon Élégance",
    vfQ: "Le texte mentionne au salon Élégance.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Clara", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Clara",
    vfQ: "Le texte mentionne Clara.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le tram 12", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le tram 12",
    vfQ: "Le texte mentionne prendre le tram 12.",
    vfC: 0,
  }),
]);
const CE_TEXT_7 = `Liste de courses

Message pour la famille ou le groupe.
acheter du pain et des fruits est prévu à 18 h, à la Coop du quartier.
ma sœur accueille les personnes.
Lisez bien jusqu'à la fin, s'il vous plaît.
Vous pouvez demander de l'aide si besoin.
Les informations importantes sont déjà notées plus haut.
Merci de votre attention et de votre patience.
Je t'envoie aussi ce détail pour être clair.
Merci : venir à pied. Détail : le total prévu est de 25 francs.`;

const CE_POOL_7 = buildExpressPool("e3-2-7", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["une liste", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "une liste",
    vfQ: "Le texte mentionne une liste.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["acheter du pain et des fruits", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "acheter du pain et des fruits",
    vfQ: "Le texte mentionne acheter du pain et des fruits.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 18 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 18 h",
    vfQ: "Le texte mentionne à 18 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la Coop du quartier", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la Coop du quartier",
    vfQ: "Le texte mentionne à la Coop du quartier.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["ma sœur", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "ma sœur",
    vfQ: "Le texte mentionne ma sœur.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_8 = `SMS du dentiste

Notification : changement autour de un contrôle dentaire.
La personne à suivre est la dentiste. Le lieu est au cabinet dentaire.
Le moment indiqué est à 9 h. Action demandée : prendre le bus.
le contrôle est couvert.
Passe le bonjour à tout le monde de ma part.
À très bientôt, prends soin de toi.
Je joins les détails importants juste après.
Dis-moi si tu as besoin d'autre chose.`;

const CE_POOL_8 = buildExpressPool("e3-2-8", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un SMS", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un SMS",
    vfQ: "Le texte mentionne un SMS.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un contrôle dentaire", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un contrôle dentaire",
    vfQ: "Le texte mentionne un contrôle dentaire.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 9 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 9 h",
    vfQ: "Le texte mentionne à 9 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au cabinet dentaire", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au cabinet dentaire",
    vfQ: "Le texte mentionne au cabinet dentaire.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["la dentiste", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "la dentiste",
    vfQ: "Le texte mentionne la dentiste.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le bus", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le bus",
    vfQ: "Le texte mentionne prendre le bus.",
    vfC: 0,
  }),
]);
const CE_TEXT_9 = `Planning de la semaine

Carte de rendez-vous.
Sujet : le cinéma. Présence de Léa et Marc.
Entrée ou arrivée au cinéma ABC mercredi à 20 h.
Avant le début, consigne : prendre le métro. Détail : le billet coûte 18 francs.
Tout le monde est le bienvenu.
Nous vous souhaitons une excellente journée.
À bientôt, et merci de votre lecture.
Voici quelques détails utiles pour la suite.`;

const CE_POOL_9 = buildExpressPool("e3-2-9", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un planning", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un planning",
    vfQ: "Le texte mentionne un planning.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["le cinéma", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "le cinéma",
    vfQ: "Le texte mentionne le cinéma.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["mercredi à 20 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "mercredi à 20 h",
    vfQ: "Le texte mentionne mercredi à 20 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au cinéma ABC", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au cinéma ABC",
    vfQ: "Le texte mentionne au cinéma ABC.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Léa et Marc", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Léa et Marc",
    vfQ: "Le texte mentionne Léa et Marc.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le métro", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le métro",
    vfQ: "Le texte mentionne prendre le métro.",
    vfC: 0,
  }),
]);
const CE_TEXT_10 = `Message de pharmacie

Règle simple : attendre mon fils.
Le sujet est chercher des médicaments. Le groupe arrive à la pharmacie du centre à 11 h.
Personne ne commence avant l'action suivante : venir à pied.
l'ordonnance est prête.
Voici quelques détails utiles pour la suite.
Lisez bien jusqu'à la fin, s'il vous plaît.
Vous pouvez demander de l'aide si besoin.
Les informations importantes sont déjà notées plus haut.
Une confirmation sera envoyée ensuite.`;

const CE_POOL_10 = buildExpressPool("e3-2-10", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un message", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un message",
    vfQ: "Le texte mentionne un message.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["chercher des médicaments", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "chercher des médicaments",
    vfQ: "Le texte mentionne chercher des médicaments.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 11 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 11 h",
    vfQ: "Le texte mentionne à 11 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la pharmacie du centre", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la pharmacie du centre",
    vfQ: "Le texte mentionne à la pharmacie du centre.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["mon fils", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "mon fils",
    vfQ: "Le texte mentionne mon fils.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_11 = `WhatsApp tennis

Organisation modifiée.
Hugo accompagne le groupe au club de tennis.
Ensuite, un match de tennis reprend à 15 h.
Pendant l'attente, action calme : venir en voiture. Détail : la raquette rouge est dans le coffre.
Dis-moi si tu as des questions, je réponds vite.
Tu peux m'appeler si c'est plus simple pour toi.
J'espère que tu vas bien et que tout se passe comme prévu.
N'oublie pas de me confirmer dès que tu peux.
Vous pouvez venir accompagné(e) si vous le souhaitez.`;

const CE_POOL_11 = buildExpressPool("e3-2-11", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un WhatsApp", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un WhatsApp",
    vfQ: "Le texte mentionne un WhatsApp.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un match de tennis", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un match de tennis",
    vfQ: "Le texte mentionne un match de tennis.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 15 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 15 h",
    vfQ: "Le texte mentionne à 15 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au club de tennis", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au club de tennis",
    vfQ: "Le texte mentionne au club de tennis.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Hugo", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Hugo",
    vfQ: "Le texte mentionne Hugo.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir en voiture", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir en voiture",
    vfQ: "Le texte mentionne venir en voiture.",
    vfC: 0,
  }),
]);
const CE_TEXT_12 = `Note sur le frigo

Besoin d'aide pour un cours de cuisine ?
La prochaine rencontre est à 18 h 30, à l'école culinaire.
un groupe de huit note les prénoms. Pour participer, action simple : prendre le bus 3.
il faut un tablier.
En cas de changement, un nouvel avis sera publié.
Nous sommes là pour vous aider.
Gardez ce texte pour vous en souvenir.
Les horaires habituels restent les mêmes.
N'oubliez pas de vérifier la date et l'heure.`;

const CE_POOL_12 = buildExpressPool("e3-2-12", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["une note", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "une note",
    vfQ: "Le texte mentionne une note.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un cours de cuisine", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un cours de cuisine",
    vfQ: "Le texte mentionne un cours de cuisine.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 18 h 30", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 18 h 30",
    vfQ: "Le texte mentionne à 18 h 30.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à l'école culinaire", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à l'école culinaire",
    vfQ: "Le texte mentionne à l'école culinaire.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["un groupe de huit", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "un groupe de huit",
    vfQ: "Le texte mentionne un groupe de huit.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le bus 3", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le bus 3",
    vfQ: "Le texte mentionne prendre le bus 3.",
    vfC: 0,
  }),
]);
const CE_TEXT_13 = `SMS bibliothèque

Madame, Monsieur, voici une information sur rendre des livres.
La présentation ou le rendez-vous est prévu à 17 h 30, à la bibliothèque municipale.
Nora confirmera demain. Consigne : venir à vélo.
aucune amende aujourd'hui.
Le contact est indiqué dans le message.
N'oubliez pas de vérifier la date.
Vous pouvez répondre directement à ce message.
Bonne journée à toutes et à tous.
Ce document complète les informations déjà données.`;

const CE_POOL_13 = buildExpressPool("e3-2-13", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un SMS", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un SMS",
    vfQ: "Le texte mentionne un SMS.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["rendre des livres", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "rendre des livres",
    vfQ: "Le texte mentionne rendre des livres.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 17 h 30", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 17 h 30",
    vfQ: "Le texte mentionne à 17 h 30.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la bibliothèque municipale", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la bibliothèque municipale",
    vfQ: "Le texte mentionne à la bibliothèque municipale.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Nora", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Nora",
    vfQ: "Le texte mentionne Nora.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à vélo", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à vélo",
    vfQ: "Le texte mentionne venir à vélo.",
    vfC: 0,
  }),
]);
const CE_TEXT_14 = `Mémo marché

Attention, changement pour le marché.
ma grand-mère ne peut pas garder l'organisation habituelle.
Rendez-vous place du Marché samedi à 8 h. Le groupe doit prendre le tram.
prendre un sac réutilisable.
Nous comptons sur vous.
Voici quelques détails utiles pour la suite.
Lisez bien jusqu'à la fin, s'il vous plaît.
Vous pouvez demander de l'aide si besoin.
À très bientôt, prends soin de toi.`;

const CE_POOL_14 = buildExpressPool("e3-2-14", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un mémo", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un mémo",
    vfQ: "Le texte mentionne un mémo.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["le marché", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "le marché",
    vfQ: "Le texte mentionne le marché.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["samedi à 8 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "samedi à 8 h",
    vfQ: "Le texte mentionne samedi à 8 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["place du Marché", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "place du Marché",
    vfQ: "Le texte mentionne place du Marché.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["ma grand-mère", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "ma grand-mère",
    vfQ: "Le texte mentionne ma grand-mère.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le tram", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le tram",
    vfQ: "Le texte mentionne prendre le tram.",
    vfC: 0,
  }),
]);
const CE_TEXT_15 = `Message vélo

Bonjour, on cherche deux personnes pour une balade à vélo.
La rencontre est au bord du lac à 10 h.
Emma apporte le matériel. Si tu viens, action demandée : venir à vélo.
la sortie est gratuite.
Les informations importantes sont déjà indiquées plus haut.
Merci de votre attention et de votre patience.
Une confirmation arrivera ensuite.
Le lieu est facile à trouver.
Prenez votre temps pour comprendre le message.`;

const CE_POOL_15 = buildExpressPool("e3-2-15", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un message", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un message",
    vfQ: "Le texte mentionne un message.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["une balade à vélo", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "une balade à vélo",
    vfQ: "Le texte mentionne une balade à vélo.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 10 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 10 h",
    vfQ: "Le texte mentionne à 10 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au bord du lac", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au bord du lac",
    vfQ: "Le texte mentionne au bord du lac.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Emma", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Emma",
    vfQ: "Le texte mentionne Emma.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à vélo", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à vélo",
    vfQ: "Le texte mentionne venir à vélo.",
    vfC: 0,
  }),
]);
const CE_TEXT_16 = `Rappel téléphone

Séance de révision ou de préparation : appeler la banque.
Mme Keller répond aux questions à 14 h.
Le groupe travaille depuis la maison, puis action finale : aucun transport.
préparer le numéro de compte.
En cas de changement, un nouvel avis sera publié.
Nous sommes là pour vous aider.
Gardez ce texte pour vous en souvenir.
Les horaires habituels restent les mêmes.
Une question ? Écrivez ou téléphonez.`;

const CE_POOL_16 = buildExpressPool("e3-2-16", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un rappel", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un rappel",
    vfQ: "Le texte mentionne un rappel.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["appeler la banque", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "appeler la banque",
    vfQ: "Le texte mentionne appeler la banque.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 14 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 14 h",
    vfQ: "Le texte mentionne à 14 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["depuis la maison", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "depuis la maison",
    vfQ: "Le texte mentionne depuis la maison.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Mme Keller", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Mme Keller",
    vfQ: "Le texte mentionne Mme Keller.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["aucun transport", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "aucun transport",
    vfQ: "Le texte mentionne aucun transport.",
    vfC: 0,
  }),
]);
const CE_TEXT_17 = `WhatsApp voisins

Petit article. Le sujet un apéro de quartier avance bien.
les voisins montre un exemple dans la cour de l'immeuble.
à 18 h, le groupe compare ses notes. Action finale : venir à pied.
apporter une boisson.
Une question ? Écrivez ou téléphonez.
Merci encore, et à bientôt.
Tout est organisé pour que ce soit simple.`;

const CE_POOL_17 = buildExpressPool("e3-2-17", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un WhatsApp", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un WhatsApp",
    vfQ: "Le texte mentionne un WhatsApp.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un apéro de quartier", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un apéro de quartier",
    vfQ: "Le texte mentionne un apéro de quartier.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 18 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 18 h",
    vfQ: "Le texte mentionne à 18 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["dans la cour de l'immeuble", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "dans la cour de l'immeuble",
    vfQ: "Le texte mentionne dans la cour de l'immeuble.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["les voisins", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "les voisins",
    vfQ: "Le texte mentionne les voisins.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_18 = `Note laverie

Bonjour à tous. Pour faire la lessive, on sort ou on se déplace.
Le rendez-vous est à la laverie automatique demain à 9 h.
Sami prend la liste. Consigne obligatoire : venir à pied.
prévoir des pièces de 2 francs.
Les informations importantes sont déjà notées plus haut.
Passe le bonjour à tout le monde.
Merci de votre attention et de votre patience.
Une confirmation arrivera ensuite.`;

const CE_POOL_18 = buildExpressPool("e3-2-18", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["une note", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "une note",
    vfQ: "Le texte mentionne une note.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["faire la lessive", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "faire la lessive",
    vfQ: "Le texte mentionne faire la lessive.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["demain à 9 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "demain à 9 h",
    vfQ: "Le texte mentionne demain à 9 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la laverie automatique", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la laverie automatique",
    vfQ: "Le texte mentionne à la laverie automatique.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["Sami", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "Sami",
    vfQ: "Le texte mentionne Sami.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir à pied", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir à pied",
    vfQ: "Le texte mentionne venir à pied.",
    vfC: 0,
  }),
]);
const CE_TEXT_19 = `SMS piscine

Nouvelle tâche : la natation.
La ressource ou l'adresse est à la piscine municipale. Fin prévue à 7 h.
David vérifiera ensuite. Pour répondre, action demandée : prendre le bus 8.
l'entrée coûte 7 francs.
N'oubliez pas de vérifier la date.
Joignez les documents demandés si nécessaire.
Bonne journée à toutes et à tous.
Ce document complète les informations déjà données.`;

const CE_POOL_19 = buildExpressPool("e3-2-19", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un SMS", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un SMS",
    vfQ: "Le texte mentionne un SMS.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["la natation", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "la natation",
    vfQ: "Le texte mentionne la natation.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["à 7 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "à 7 h",
    vfQ: "Le texte mentionne à 7 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["à la piscine municipale", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "à la piscine municipale",
    vfQ: "Le texte mentionne à la piscine municipale.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["David", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "David",
    vfQ: "Le texte mentionne David.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["prendre le bus 8", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "prendre le bus 8",
    vfQ: "Le texte mentionne prendre le bus 8.",
    vfC: 0,
  }),
]);
const CE_TEXT_20 = `Planning du dimanche

Journée spéciale autour de un brunch.
Le matin, passage au café du Port avec mes amis.
La présentation commence dimanche à 10 h. Chaque groupe doit venir en voiture.
la réservation est pour quatre.
Ce document complète les informations déjà données.
Nous comptons sur vous.
Le service répond en français et en anglais.
Une version audio est disponible sur demande.
Conservez le numéro de contact indiqué.`;

const CE_POOL_20 = buildExpressPool("e3-2-20", [
  q({
    id: "ce-q1",
    textQ: "Quel type de document est-ce ?",
    text: ["un planning", "une facture", "une recette"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "un planning",
    vfQ: "Le texte mentionne un planning.",
    vfC: 0,
  }),
  q({
    id: "ce-q2",
    textQ: "Quelle activité est mentionnée ?",
    text: ["un brunch", "un examen", "dormir"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "L'activité est _________.",
    fill: "un brunch",
    vfQ: "Le texte mentionne un brunch.",
    vfC: 0,
  }),
  q({
    id: "ce-q3",
    textQ: "Quand cela se passe-t-il ?",
    text: ["dimanche à 10 h", "à minuit", "en 1990"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "C'est _________.",
    fill: "dimanche à 10 h",
    vfQ: "Le texte mentionne dimanche à 10 h.",
    vfC: 0,
  }),
  q({
    id: "ce-q4",
    textQ: "Où cela se passe-t-il ?",
    text: ["au café du Port", "à la plage", "à l'aéroport"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le lieu est _________.",
    fill: "au café du Port",
    vfQ: "Le texte mentionne au café du Port.",
    vfC: 0,
  }),
  q({
    id: "ce-q5",
    textQ: "Qui est mentionné ?",
    text: ["mes amis", "un voisin", "le facteur"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "La personne est _________.",
    fill: "mes amis",
    vfQ: "Le texte mentionne mes amis.",
    vfC: 0,
  }),
  q({
    id: "ce-q6",
    textQ: "Quel transport est mentionné ?",
    text: ["venir en voiture", "l'avion", "le bateau"],
    textC: 0,
    img: ["", "", ""],
    imgC: 0,
    fillQ: "Le transport est _________.",
    fill: "venir en voiture",
    vfQ: "Le texte mentionne venir en voiture.",
    vfC: 0,
  }),
]);

export const E3_2_CE: CommunicationExercise[] = [
  readingPoolExercise({
  id: "e3-2-1",
  readingText: CE_TEXT_1,
  questionPool: CE_POOL_1
}),
  readingPoolExercise({
  id: "e3-2-2",
  readingText: CE_TEXT_2,
  questionPool: CE_POOL_2
}),
  readingPoolExercise({
  id: "e3-2-3",
  readingText: CE_TEXT_3,
  questionPool: CE_POOL_3
}),
  readingPoolExercise({
  id: "e3-2-4",
  readingText: CE_TEXT_4,
  questionPool: CE_POOL_4
}),
  readingPoolExercise({
  id: "e3-2-5",
  readingText: CE_TEXT_5,
  questionPool: CE_POOL_5
}),
  readingPoolExercise({
  id: "e3-2-6",
  readingText: CE_TEXT_6,
  questionPool: CE_POOL_6
}),
  readingPoolExercise({
  id: "e3-2-7",
  readingText: CE_TEXT_7,
  questionPool: CE_POOL_7
}),
  readingPoolExercise({
  id: "e3-2-8",
  readingText: CE_TEXT_8,
  questionPool: CE_POOL_8
}),
  readingPoolExercise({
  id: "e3-2-9",
  readingText: CE_TEXT_9,
  questionPool: CE_POOL_9
}),
  readingPoolExercise({
  id: "e3-2-10",
  readingText: CE_TEXT_10,
  questionPool: CE_POOL_10
}),
  readingPoolExercise({
  id: "e3-2-11",
  readingText: CE_TEXT_11,
  questionPool: CE_POOL_11
}),
  readingPoolExercise({
  id: "e3-2-12",
  readingText: CE_TEXT_12,
  questionPool: CE_POOL_12
}),
  readingPoolExercise({
  id: "e3-2-13",
  readingText: CE_TEXT_13,
  questionPool: CE_POOL_13
}),
  readingPoolExercise({
  id: "e3-2-14",
  readingText: CE_TEXT_14,
  questionPool: CE_POOL_14
}),
  readingPoolExercise({
  id: "e3-2-15",
  readingText: CE_TEXT_15,
  questionPool: CE_POOL_15
}),
  readingPoolExercise({
  id: "e3-2-16",
  readingText: CE_TEXT_16,
  questionPool: CE_POOL_16
}),
  readingPoolExercise({
  id: "e3-2-17",
  readingText: CE_TEXT_17,
  questionPool: CE_POOL_17
}),
  readingPoolExercise({
  id: "e3-2-18",
  readingText: CE_TEXT_18,
  questionPool: CE_POOL_18
}),
  readingPoolExercise({
  id: "e3-2-19",
  readingText: CE_TEXT_19,
  questionPool: CE_POOL_19
}),
  readingPoolExercise({
  id: "e3-2-20",
  readingText: CE_TEXT_20,
  questionPool: CE_POOL_20
}),
];

/* ── Production orale — dialogues à jouer (thème quotidien) ───────────────── */

const AMI = { title: "L'ami", vous: "l'ami / l'amie" };
const COLLEGUE = { title: "Le collègue", vous: "le collègue / la collègue" };

export const E3_2_PO: ExpressPoDialogue[] = [
  {
    id: "e3-2-po-1",
    title: "En retard le matin",
    context: "C'est le matin, vous êtes en retard pour l'école et votre mère vous appelle.",
    roleA: { title: "La mère", vous: "la mère / le père" },
    roleB: { title: "L'enfant", vous: "l'enfant" },
    lines: [
      { role: "A", text: "Dépêche-toi, on est en retard !" },
      { role: "B", text: "Maman, je prends mon petit déjeuner !" },
      { role: "A", text: "Tu te couches trop tard le soir, c'est ça le problème." },
      { role: "B", text: "D'accord… Ce soir, je me couche tôt." },
      { role: "A", text: "Et mets ton réveil à 6 h 30 !" },
      { role: "B", text: "Promis. Tu peux préparer mon sac, s'il te plaît ?" },
      { role: "A", text: "Non, tu prépares ton sac ce soir, avant de dormir." },
      { role: "B", text: "Bon, d'accord. J'arrive dans deux minutes !" },
      { role: "A", text: "Enchanté(e) de te connaître !" },
      { role: "B", text: "Moi aussi. À tout à l'heure !" },
],
  },
  {
    id: "e3-2-po-2",
    title: "Parler de sa routine",
    context: "Vous parlez de votre matin avec un ami.",
    roleA: AMI,
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "Tu te lèves à quelle heure le matin ?" },
      { role: "B", text: "À 6 h 30. Je me douche et je m'habille." },
      { role: "A", text: "Et tu prends un petit déjeuner ?" },
      { role: "B", text: "Oui, toujours. Un café et du pain. Et toi ?" },
      { role: "A", text: "Moi, je me lève à 7 heures et je pars vite au travail." },
      { role: "B", text: "Sans manger ? Ce n'est pas bien !" },
      { role: "A", text: "Je sais… Je mange un fruit au bureau." },
      { role: "B", text: "Demain, on prend le petit déjeuner ensemble !" },
      { role: "A", text: "Enchanté(e) de te connaître !" },
      { role: "B", text: "Moi aussi. À tout à l'heure !" },
],
  },
  {
    id: "e3-2-po-3",
    title: "Le week-end",
    context: "Un ami vous demande ce que vous faites le week-end.",
    roleA: AMI,
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "Qu'est-ce que tu fais le week-end, d'habitude ?" },
      { role: "B", text: "Le samedi, je me lève tard, vers 10 heures." },
      { role: "A", text: "Et après ?" },
      { role: "B", text: "Je me promène en ville ou je fais du sport." },
      { role: "A", text: "Tu sors le samedi soir ?" },
      { role: "B", text: "Oui, je dîne au restaurant avec des amis." },
      { role: "A", text: "Et le dimanche ?" },
      { role: "B", text: "Le dimanche, on invite la famille à déjeuner." },
      { role: "A", text: "Merci pour votre aide." },
      { role: "B", text: "Je vous en prie. Bonne journée !" },
],
  },
  {
    id: "e3-2-po-4",
    title: "Petit déjeuner ensemble",
    context: "Une amie vous propose un petit déjeuner demain matin, mais c'est le week-end.",
    roleA: { title: "L'amie", vous: "l'ami / l'amie" },
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "On prend le petit déjeuner ensemble demain matin ?" },
      { role: "B", text: "Avec plaisir ! À quelle heure ?" },
      { role: "A", text: "À huit heures et demie, ça va ?" },
      { role: "B", text: "C'est trop tôt ! Demain, c'est samedi, je me lève à 10 heures." },
      { role: "A", text: "Alors mets ton réveil !" },
      { role: "B", text: "Ah non, pas de réveil le week-end !" },
      { role: "A", text: "Bon, d'accord. Alors on déjeune ensemble à midi ?" },
      { role: "B", text: "À midi, parfait !" },
      { role: "A", text: "Merci, c'est gentil." },
      { role: "B", text: "De rien. Au revoir !" },
],
  },
  {
    id: "e3-2-po-5",
    title: "Au club de sport",
    context: "Vous travaillez beaucoup et vous voulez vous inscrire dans un club de sport.",
    roleA: { title: "L'employé du club", vous: "l'employé / l'employée du club" },
    roleB: { title: "Le client", vous: "le client / la cliente" },
    lines: [
      { role: "A", text: "Bonjour, bienvenue au club Athletic !" },
      { role: "B", text: "Bonjour, je voudrais m'inscrire. Je travaille beaucoup et je suis fatigué." },
      { role: "A", text: "Bonne idée ! Vous avez du temps libre quand ?" },
      { role: "B", text: "Le soir, après le travail, vers 18 heures." },
      { role: "A", text: "Parfait, nous avons des cours de sport tous les soirs." },
      { role: "B", text: "Il y a aussi des cours à l'heure du déjeuner ?" },
      { role: "A", text: "Oui, de midi à 13 heures." },
      { role: "B", text: "Super, je m'inscris pour le soir !" },
      { role: "A", text: "Ravi(e) de faire votre connaissance." },
      { role: "B", text: "Moi aussi. À bientôt !" },
],
  },
  {
    id: "e3-2-po-6",
    title: "Le soir à la maison",
    context: "Vous parlez de votre soirée avec votre colocataire.",
    roleA: { title: "Le colocataire", vous: "le colocataire / la colocataire" },
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "Qu'est-ce que tu fais ce soir ?" },
      { role: "B", text: "Je me repose, je suis fatigué. Et toi ?" },
      { role: "A", text: "Moi, je regarde une série après le dîner." },
      { role: "B", text: "Tu regardes trop la télé ! Tu peux lire aussi." },
      { role: "A", text: "C'est vrai… Et tu te couches à quelle heure ?" },
      { role: "B", text: "À 22 heures. Demain, je me lève tôt." },
      { role: "A", text: "D'accord. Alors on dîne maintenant ?" },
      { role: "B", text: "Oui, le repas est prêt !" },
      { role: "A", text: "Super, merci beaucoup." },
      { role: "B", text: "Avec plaisir. Bonne continuation !" },
],
  },
  {
    id: "e3-2-po-7",
    title: "Toujours à l'heure",
    context: "Votre collègue arrive toujours à l'heure au travail. Vous lui demandez son secret.",
    roleA: { title: "Vous", vous: "vous-même" },
    roleB: COLLEGUE,
    lines: [
      { role: "A", text: "Salut ! Toujours à l'heure, toi !" },
      { role: "B", text: "Oui, je n'aime pas être en retard." },
      { role: "A", text: "Comment tu fais ? Moi, j'arrive souvent en retard." },
      { role: "B", text: "Je me réveille tôt et je prépare mes affaires le soir." },
      { role: "A", text: "Tu te lèves à quelle heure ?" },
      { role: "B", text: "À 6 heures. J'arrive au bureau en avance, à 7 h 30." },
      { role: "A", text: "Bravo ! Demain, je mets mon réveil à 6 heures aussi." },
      { role: "B", text: "Bonne idée ! À demain !" },
      { role: "A", text: "Bon courage pour la suite !" },
      { role: "B", text: "Merci, toi aussi !" },
],
  },
  {
    id: "e3-2-po-8",
    title: "La journée de travail",
    context: "Vous parlez de votre fin de journée avec un ami et vous organisez le week-end.",
    roleA: AMI,
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "Tu finis le travail à quelle heure aujourd'hui ?" },
      { role: "B", text: "À 17 heures. Après, je fais les courses." },
      { role: "A", text: "Et le soir, tu cuisines ?" },
      { role: "B", text: "Oui, je rentre à la maison et je prépare le dîner." },
      { role: "A", text: "Tu as du temps libre en semaine ?" },
      { role: "B", text: "Pas beaucoup, je travaille beaucoup." },
      { role: "A", text: "Alors on se voit samedi ? On se promène au parc ?" },
      { role: "B", text: "D'accord, samedi après-midi !" },
      { role: "A", text: "Bon courage pour la suite !" },
      { role: "B", text: "Merci, toi aussi !" },
],
  },
  {
    id: "e3-2-po-9",
    title: "Une nouvelle habitude",
    context: "Vous vous levez tôt pour faire du sport et un ami vous pose des questions.",
    roleA: AMI,
    roleB: { title: "Vous", vous: "vous-même" },
    lines: [
      { role: "A", text: "Tu as l'air en forme ! Qu'est-ce qui change ?" },
      { role: "B", text: "Maintenant, je me lève à 6 heures et je fais du sport." },
      { role: "A", text: "Tous les matins ? C'est difficile, non ?" },
      { role: "B", text: "Au début, oui. Maintenant, j'adore ça." },
      { role: "A", text: "Et tu te couches à quelle heure ?" },
      { role: "B", text: "À 22 heures. Je dors huit heures par nuit." },
      { role: "A", text: "Moi aussi, je veux essayer !" },
      { role: "B", text: "Super ! On court ensemble demain matin ?" },
      { role: "A", text: "Merci pour votre aide." },
      { role: "B", text: "Je vous en prie. Bonne journée !" },
],
  },
  {
    id: "e3-2-po-10",
    title: "Interview sur le quotidien",
    context: "Un journaliste du journal de la ville vous pose des questions sur votre quotidien.",
    roleA: { title: "Le journaliste", vous: "le journaliste / la journaliste" },
    roleB: { title: "Le passant", vous: "le passant / la passante" },
    lines: [
      { role: "A", text: "Bonjour ! Je peux vous poser des questions sur votre quotidien ?" },
      { role: "B", text: "Oui, bien sûr." },
      { role: "A", text: "Vous vous levez à quelle heure le matin ?" },
      { role: "B", text: "À 7 heures. Je me prépare en trente minutes." },
      { role: "A", text: "Et que faites-vous le soir ?" },
      { role: "B", text: "Je me promène avec mon chien, puis je lis un peu." },
      { role: "A", text: "Merci beaucoup ! Dernière question : le week-end ?" },
      { role: "B", text: "Le week-end, je me repose et je vois mes amis !" },
      { role: "A", text: "Ravi(e) de faire votre connaissance." },
      { role: "B", text: "Moi aussi. À bientôt !" },
],
  },
{
  id: "e3-2-po-11",
  title: "Demander une information sur la ville",
  context: "Vous parlez en français simple. Vous devez demander une information sur la ville : trouver la bibliothèque municipale.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, je peux vous renseigner ?" },
    { role: "B", text: "Bonjour, je voudrais savoir comment faire pour trouver la bibliothèque municipale." },
    { role: "A", text: "Bien sûr. Elle est derrière la poste." },
    { role: "B", text: "D'accord. Est-ce qu'il faut réserver avant ?" },
    { role: "A", text: "Oui. Elle ferme à 19 heures." },
    { role: "B", text: "Très bien. Je peux le faire aujourd'hui ?" },
    { role: "A", text: "Oui, si vous avez quelques minutes." },
    { role: "B", text: "Parfait, je commence tout de suite." },
    { role: "A", text: "Je reste disponible si vous avez une question." },
    { role: "B", text: "Merci beaucoup pour votre aide." },
  ],
},
{
  id: "e3-2-po-12",
  title: "Expliquer un problème avec la ville",
  context: "Vous parlez en français simple. Vous devez expliquer un problème avec la ville : le lampadaire devant chez moi ne fonctionne plus.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, qu'est-ce qui se passe ?" },
    { role: "B", text: "Bonjour, j'ai un problème : le lampadaire devant chez moi ne fonctionne plus." },
    { role: "A", text: "Je comprends. Nous allons signaler la panne." },
    { role: "B", text: "Merci. Est-ce possible de régler ça maintenant ?" },
    { role: "A", text: "Un technicien passe cette semaine." },
    { role: "B", text: "Très bien, merci." },
    { role: "A", text: "Je m'en occupe tout de suite." },
    { role: "B", text: "C'est gentil, je vous remercie." },
    { role: "A", text: "Je vous en prie." },
    { role: "B", text: "Merci, bonne journée." },
  ],
},
{
  id: "e3-2-po-13",
  title: "Prendre rendez-vous pour la ville",
  context: "Vous parlez en français simple. Vous devez prendre rendez-vous pour la ville : rencontrer le service logement de la ville.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, vous souhaitez un rendez-vous ?" },
    { role: "B", text: "Oui, je voudrais rencontrer le service logement de la ville." },
    { role: "A", text: "Je peux vous proposer mardi à 15 heures." },
    { role: "B", text: "Oui, ce créneau me convient." },
    { role: "A", text: "J'apporte mon dossier." },
    { role: "B", text: "D'accord, je le note." },
    { role: "A", text: "Je vous envoie une confirmation par message." },
    { role: "B", text: "Merci. Vous avez mon numéro ?" },
    { role: "A", text: "Oui, il est noté ici." },
    { role: "B", text: "Parfait, à bientôt." },
  ],
},
{
  id: "e3-2-po-14",
  title: "Confirmer un rendez-vous",
  context: "Vous parlez en français simple. Vous devez confirmer un rendez-vous : la visite guidée du centre.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, vous appelez pour confirmer ?" },
    { role: "B", text: "Oui, je confirme la visite guidée du centre." },
    { role: "A", text: "Très bien. C'est bien samedi à 10 heures ?" },
    { role: "B", text: "Oui, c'est exact." },
    { role: "A", text: "Parfait, c'est noté." },
    { role: "B", text: "Je viens avec mon voisin." },
    { role: "A", text: "Oui, aucun problème." },
    { role: "B", text: "Merci, c'est très clair." },
    { role: "A", text: "Parfait, à ce moment-là." },
    { role: "B", text: "Merci, à bientôt." },
  ],
},
{
  id: "e3-2-po-15",
  title: "Demander conseil sur la ville",
  context: "Vous parlez en français simple. Vous devez demander conseil sur la ville : choisir un quartier calme.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Vous voulez un conseil ?" },
    { role: "B", text: "Oui, j'hésite pour choisir un quartier calme." },
    { role: "A", text: "Près du parc c'est agréable." },
    { role: "B", text: "C'est vrai, mais je veux aussi quelque chose de pratique." },
    { role: "A", text: "Près de la gare c'est plus bruyant." },
    { role: "B", text: "Je comprends mieux la différence." },
    { role: "A", text: "Choisissez ce qui vous simplifie la vie." },
    { role: "B", text: "Vous avez raison. Je vais y réfléchir ce soir." },
    { role: "A", text: "Revenez me voir si vous voulez en reparler." },
    { role: "B", text: "Merci pour votre conseil." },
  ],
},
{
  id: "e3-2-po-16",
  title: "Signaler un retard",
  context: "Vous parlez en français simple. Vous devez signaler un retard : j'arrive en retard à la visite.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, je vous écoute." },
    { role: "B", text: "Bonjour, je vous préviens : j'arrive en retard à la visite." },
    { role: "A", text: "Merci de nous prévenir. Que se passe-t-il ?" },
    { role: "B", text: "Je me suis trompé de bus." },
    { role: "A", text: "D'accord, ce n'est pas grave." },
    { role: "B", text: "Je rejoins le groupe à la mairie." },
    { role: "A", text: "Très bien, nous vous attendons." },
    { role: "B", text: "Merci pour votre compréhension." },
    { role: "A", text: "À tout à l'heure." },
    { role: "B", text: "À tout à l'heure." },
  ],
},
{
  id: "e3-2-po-17",
  title: "Faire une réclamation polie",
  context: "Vous parlez en français simple. Vous devez faire une réclamation polie : les poubelles de ma rue débordent.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, comment puis-je vous aider ?" },
    { role: "B", text: "Bonjour, je voudrais signaler un problème : les poubelles de ma rue débordent." },
    { role: "A", text: "Je suis désolé pour cela." },
    { role: "B", text: "L'odeur est forte." },
    { role: "A", text: "Je comprends. Quelle solution souhaitez-vous ?" },
    { role: "B", text: "Pouvez-vous demander un passage ?" },
    { role: "A", text: "D'accord, je vais transmettre votre demande." },
    { role: "B", text: "Merci. J'aimerais être informé rapidement." },
    { role: "A", text: "Je vous réponds dès que possible." },
    { role: "B", text: "Très bien, merci." },
  ],
},
{
  id: "e3-2-po-18",
  title: "Demander une aide urgente",
  context: "Vous parlez en français simple. Vous devez demander une aide urgente : trouver une pharmacie ouverte ce soir.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, c'est urgent ?" },
    { role: "B", text: "Oui, j'ai besoin d'aide pour trouver une pharmacie ouverte ce soir." },
    { role: "A", text: "Celle de la gare est de garde." },
    { role: "B", text: "D'accord, je fais ça tout de suite." },
    { role: "A", text: "Elle ferme à 22 heures." },
    { role: "B", text: "Merci. Est-ce que je dois rappeler ?" },
    { role: "A", text: "Non, venez directement si besoin." },
    { role: "B", text: "Très bien, je pars maintenant." },
    { role: "A", text: "Bon courage." },
    { role: "B", text: "Merci beaucoup." },
  ],
},
{
  id: "e3-2-po-19",
  title: "Comparer deux possibilités",
  context: "Vous parlez en français simple. Vous devez comparer deux possibilités : prendre le bus ou marcher jusqu'au marché.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Vous hésitez entre deux possibilités ?" },
    { role: "B", text: "Oui, je compare prendre le bus ou marcher jusqu'au marché." },
    { role: "A", text: "À pied c'est quinze minutes." },
    { role: "B", text: "Et l'autre possibilité ?" },
    { role: "A", text: "Le bus est utile avec des sacs." },
    { role: "B", text: "Je vois. Je cherche surtout quelque chose de pratique." },
    { role: "A", text: "Dans ce cas, la première option est peut-être meilleure." },
    { role: "B", text: "D'accord, je vais choisir celle-là." },
    { role: "A", text: "Très bien, je vous prépare ça." },
    { role: "B", text: "Merci pour vos conseils." },
  ],
},
{
  id: "e3-2-po-20",
  title: "Remercier pour une aide",
  context: "Vous parlez en français simple. Vous devez remercier pour une aide : vos informations sur la ville.",
  roleA: { title: "L'agent d'accueil", vous: "l'agent / l'agente d'accueil" },
  roleB: { title: "Vous", vous: "la personne qui parle" },
  lines: [
    { role: "A", text: "Bonjour, vous vouliez me parler ?" },
    { role: "B", text: "Oui, je voulais vous remercier pour vos informations sur la ville." },
    { role: "A", text: "C'est gentil, merci." },
    { role: "B", text: "J'ai trouvé la piscine facilement." },
    { role: "A", text: "Je suis content que cela vous ait aidé." },
    { role: "B", text: "Vos explications étaient très claires." },
    { role: "A", text: "N'hésitez pas à revenir si besoin." },
    { role: "B", text: "Oui, je le ferai." },
    { role: "A", text: "Bonne continuation !" },
    { role: "B", text: "Merci, à vous aussi." },
  ],
}
];

/* ── Production écrite — consignes (A1 : 50 mots minimum) ─────────────────── */

const PE_MIN = 50;
const PE_MAX = 120;

export const E3_2_PE: ExpressPePrompt[] = [
  {
    id: "e3-2-pe-1",
    title: "Journée de semaine",
    situation: "",
    instruction: "Décrivez une journée de votre semaine. Commencez par le matin, dites à quelle heure vous vous réveillez et racontez vos activités jusqu'au soir.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-2",
    title: "Activités des Français",
    situation: "",
    instruction: "Vous répondez par écrit à un ami qui demande des informations sur les activités quotidiennes des Français. Décrivez une journée simple avec les repas, le travail et les loisirs.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-3",
    title: "Matin presse",
    situation: "",
    instruction: "Racontez un matin où vous êtes presse. Dites à quelle heure vous vous levez, ce que vous faites vite et comment vous partez.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-4",
    title: "Soirée calme",
    situation: "",
    instruction: "Décrivez votre soirée après le travail ou les cours. Parlez du repas, des activités à la maison et de l'heure du coucher.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-5",
    title: "Routine du week-end",
    situation: "",
    instruction: "Écrivez un texte sur votre routine du samedi. Dites ce qui change par rapport à la semaine et ce que vous aimez faire.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-6",
    title: "Journée idéale",
    situation: "",
    instruction: "Décrivez votre journée idéale en phrases simples. Parlez du matin, de l'après-midi, du soir et des personnes avec vous.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-7",
    title: "Habitudes de repas",
    situation: "",
    instruction: "Expliquez vos habitudes de repas pendant une journée normale. Dites ce que vous mangez au petit déjeuner, au déjeuner et au dîner.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-8",
    title: "Transport quotidien",
    situation: "",
    instruction: "Racontez votre trajet quotidien. Dites comment vous allez au travail ou à l'école, combien de temps cela prend et ce que vous faites pendant le trajet.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-9",
    title: "Activité sportive",
    situation: "",
    instruction: "Décrivez une journée où vous faites du sport. Indiquez l'heure, le lieu, l'activite et ce que vous faites après.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-10",
    title: "Journée avec enfants",
    situation: "",
    instruction: "Racontez une journée avec des enfants. Parlez du réveil, de l'école, des repas et des activités du soir.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-11",
    title: "Routine de vacances",
    situation: "",
    instruction: "Décrivez votre routine pendant les vacances. Dites à quelle heure vous vous levez, ce que vous visitez et comment vous vous reposez.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-12",
    title: "Planning charge",
    situation: "",
    instruction: "Vous avez une journée très chargee. Écrivez un message pour expliquer vos rendez-vous, vos horaires et le moment où vous êtes libre.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-13",
    title: "Matin en France",
    situation: "",
    instruction: "Expliquez à un ami comment se passe souvent un matin en France. Parlez du petit déjeuner, du transport et du debut de la journée.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-14",
    title: "Comparaison de routines",
    situation: "",
    instruction: "Comparez votre routine de semaine et votre routine du dimanche. Dites ce qui est pareil et ce qui est différent.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-15",
    title: "Retard au rendez-vous",
    situation: "",
    instruction: "Vous êtes en retard parce que votre routine du matin à change. Écrivez un message pour expliquer ce qui s'est passe et donner votre heure d'arrivee.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-16",
    title: "Routine de colocataire",
    situation: "",
    instruction: "Décrivez la routine de votre colocataire. Parlez de ses horaires, de ses habitudes et d'une activité qu'il fait tous les jours.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-17",
    title: "Après les cours",
    situation: "",
    instruction: "Racontez ce que vous faites après les cours. Dites où vous allez, avec qui, et à quelle heure vous rentrez.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-18",
    title: "Journée sans travail",
    situation: "",
    instruction: "Décrivez une journée sans travail. Parlez du réveil, des courses, des loisirs et d'une chose agreable.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-19",
    title: "Objectif nouvelle routine",
    situation: "",
    instruction: "Écrivez un message pour expliquer une nouvelle routine que vous voulez essayer. Dites ce que vous changez et pourquoi.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },
  {
    id: "e3-2-pe-20",
    title: "Fin de journée",
    situation: "",
    instruction: "Décrivez la fin de votre journée. Parlez du dîner, de la famille ou des amis, des ecrans et du sommeil.",
    points: [],
    minWords: PE_MIN,
    maxWords: PE_MAX,
  },

];
