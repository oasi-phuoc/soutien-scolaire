...
const T: Record<string, Record<"fr", string> & Partial<Record<Lang, string>>> = {
  ...
  tapEachLetter:       { fr: "Coloriez les cases avec la lettre {x}", en: "Colour the boxes with the letter {x}", ar: "لوّن الخانات التي فيها الحرف {x}", fa: "خانه‌های حرف {x} را رنگ کن", pt: "Pinte as casas com a letra {x}", so: "Midabee sanduuqyada xarafka {x}", ti: "ነቲ ፊደል {x} ዘለዎ ሳንዱቕ ሕብር ግበሩ", uk: "Зафарбуй клітинки з літерою {x}", tr: "{x} harfi olan kutuları boyayın", ps: "د {x} توري خانې رنګ کړئ" },
  // WordSpotter
  ...
  tapLetterInWords:    { fr: "Soulignez les lettres {x}", en: "Underline the letters {x}", ar: "ضع خطاً تحت الحروف {x}", fa: "حروف {x} را خط بکش", pt: "Sublinhe as letras {x}", so: "Hoos ka xarriiq xarfaha {x}", ti: "ፊደላት {x} ትሕቲ ስመር", uk: "Підкресли літери {x}", tr: "{x} harflerinin altını çizin", ps: "د {x} تورو لاندې کرښه وکړئ" },
  // SoundPicker
  ...
  tapImagesWithSound:  { fr: "Cochez si vous entendez le son {x}", en: "Tick if you hear the sound {x}", ar: "ضع علامة إذا سمعت الصوت {x}", fa: "اگر صدای {x} را می‌شنوی علامت بزن", pt: "Assinale se ouve o som {x}", so: "Calaamadee haddii aad maqasho codka {x}", ti: "ድምጺ {x} እንተሰሚዕካ ምልክት ግበር", uk: "Познач, якщо чуєш звук {x}", tr: "{x} sesini duyuyorsanız işaretleyin", ps: "که غږ {x} اورئ نښه یې کړئ" },
  listenTapWithSound:  { fr: "Cochez si vous entendez le son {x}", en: "Tick if you hear the sound {x}", ar: "ضع علامة إذا سمعت الصوت {x}", fa: "اگر صدای {x} را می‌شنوی علامت بزن", pt: "Assinale se ouve o som {x}", so: "Calaamadee haddii aad maqasho codka {x}", ti: "ድምጺ {x} እንተሰሚዕካ ምልክት ግበር", uk: "Познач, якщо чуєш звук {x}", tr: "{x} sesini duyuyorsanız işaretleyin", ps: "که غږ {x} اورئ نښه یې کړئ" },
  ...
  tapSyllableAloud:    { fr: "Écoutez et répétez les syllabes.", en: "Listen and repeat the syllables.", ar: "استمع وكرر المقاطع.", fa: "گوش بده و هجاها را تکرار کن.", pt: "Ouça e repita as sílabas.", so: "Dhagayso oo ku celi shaqallada.", ti: "ስምዑን ድገሙን ነቶም ምምቕቓላት.", uk: "Слухай і повторюй склади.", tr: "Heceleri dinleyin ve tekrar edin.", ps: "هجاوې واورئ او تکرار یې کړئ." },
  // RevisionLetterGrid
