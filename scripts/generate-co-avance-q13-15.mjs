#!/usr/bin/env node
/** Génère scripts/co-avance-q13-15.json et valide chaque question. */
import fs from "node:fs";
import { createRequire } from "node:module";

const require = createRequire(import.meta.url);
const { isExcludedCeCoQuestion } = require("../lib/curriculum/content/communication/ce-co-question-filters.ts");
...
const EXPECTED_POOLS = [
  ...Array.from({ length: 7 }, (_, i) => `scolaire-annonce-${30 + i}`),
  ...Array.from({ length: 14 }, (_, i) => `scolaire-conversation-${i + 1}`),
  ...Array.from({ length: 15 }, (_, i) => `scolaire-radio-${15 + i}`),
];

let excluded = [];
let total = 0;

for (const pool of EXPECTED_POOLS) {
  const questions = DATA[pool];
  if (!questions) {
    console.error(`Pool manquant: ${pool}`);
    process.exit(1);
  }
  if (questions.length !== 3) {
    console.error(`${pool}: attendu 3 questions, trouvé ${questions.length}`);
    process.exit(1);
  }
  for (const q of questions) {
    total++;
    if (isExcludedCeCoQuestion(q)) {
      excluded.push(`${q.id}: ${q.textQ}`);
    }
  }
}

if (excluded.length) {
  console.error("Questions exclues par le filtre:\n" + excluded.join("\n"));
  process.exit(1);
}

const outPath = "scripts/co-avance-q13-15.json";
fs.writeFileSync(outPath, JSON.stringify(DATA, null, 2) + "\n");
console.log(`Écrit ${outPath}: ${Object.keys(DATA).length} pools, ${total} questions`);
