...
function collectCE() {
  ...
  const paths = new Map();
  for (const match of src.matchAll(/\/expression\/ce\/(?:base|moyen|avance)\/([^`"' )]+\.webp)/g)) {
    const rel = match[1];
    if (!rel.includes("${")) paths.set(rel, rel.replace(/\.webp$/, "").replace(/[-/]/g, " "));
  }
...
function collectObjectPick() {
  const src = fs.readFileSync(
    path.join(ROOT, "lib/curriculum/content/communication/co-questions-objet-pick.ts"),
    "utf8",
  );
  let count = 0;
  const groupRegex = /opCards\("([^"]+)",\s*\[([\s\S]*?)\]\)/g;
  for (const match of src.matchAll(groupRegex)) {
