...
def _message_patterns(s: dict, idx: int) -> dict:
    patterns = [
        lambda: _entry(
            ...
            [
                f"Rendez-vous {s['date']} à {s['time']} : {s['place']} ouvre ses portes pour {s['activity']}.",
                f"{_cap(s['person'])} accueille le groupe. {_price_sentence(s['price'])}",
...
def _email_patterns(s: dict, idx: int) -> dict:
    ...
    patterns = [
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"C'est confirmé : vous avez une place pour {s['activity']}.",
                f"Notez bien le rendez-vous : {s['date']}, {s['time']}, à {s['place']}.",
        ...
        lambda: _email(
            ...
            [
                f"Je vous rappelle que {s['activity']} est prévu {s['date']}.",
                f"Le groupe se retrouve à {s['time']} devant {s['place']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"J'ai vu l'annonce pour {s['activity']} et je voudrais m'inscrire.",
                f"Pouvez-vous confirmer qu'il faut apporter {s['item']} ?",
        ...
        lambda: _email(
            ...
            "Madame, Monsieur,",
            [
                f"Votre place pour {s['activity']} est réservée.",
                f"La séance aura lieu {s['date']} à {s['place']}.",
        ...
        lambda: _email(
            ...
            "Salut,",
            [
                f"Petit conseil pour {s['activity']} : mets {s['item']} dans ton sac.",
                f"Je t'attends {s['date']} à {s['time']} à {s['place']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Petit changement pour {s['activity']} : la salle prévue n'est plus libre.",
                f"Le nouveau lieu est {s['place']}. La date et l'heure ne changent pas : {s['date']} à {s['time']}.",
                f"Le prix reste {s['price']} et les {s['limit']} sont gardées.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Merci pour votre présence lors de {s['activity']} à {s['place']}.",
                f"Plusieurs personnes ont expliqué que la séance aide à {s['benefit']}.",
        ...
        lambda: _email(
            ...
            "Bonjour les amis,",
            [
                f"Ça vous dit de venir avec moi à {s['activity']} ?",
                f"Le rendez-vous est {s['date']} à {s['time']} à {s['place']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Pour {s['activity']}, la petite liste est simple : {s['item']}, une bouteille d'eau et un stylo.",
                f"Le rendez-vous reste {s['date']} à {s['time']} à {s['place']}.",
                f"Le tarif est {s['price']}.",
                f"Le thème principal sera : {s['benefit']}.",
            ],
            "Merci,",
            sender,
            base_q,
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Bonne nouvelle : {s['activity']} n'est pas complet, il reste {s['limit']}.",
                f"La séance aura lieu {s['date']} à {s['time']} à {s['place']}.",
                f"Merci de vous inscrire {s['contact']} avant demain soir.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Depuis {s['activity']}, nous recevons des messages très positifs.",
                f"Les participants ont surtout apprécié de {s['benefit']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Votre rendez-vous lié à {s['activity']} commence à {s['time']}.",
                f"Merci d'arriver à {s['place']} dix minutes avant.",
        ...
        lambda: _email(
            ...
            "Chers participants,",
            [
                f"Pour que {s['activity']} se passe bien, nous formons deux petits groupes.",
                f"Le premier groupe est attendu à {s['time']} à {s['place']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Je vous écris car je cherche des informations sur {s['activity']}.",
                f"J'ai noté la date, {s['date']}, et l'heure, {s['time']}, à {s['place']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Après vos réponses, nous gardons finalement la date du {s['date']}.",
                f"L'horaire choisi est {s['time']} à {s['place']}.",
                f"Ce choix convient mieux à {s['person']} et au groupe.",
                f"Le contenu reste le même : {s['activity']}, avec {s['item']} à prévoir.",
            ],
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Dernier message avant {s['activity']} !",
                f"Rendez-vous {s['date']} à {s['time']} à {s['place']}.",
                f"L'entrée n'est garantie que si l'inscription est faite {s['contact']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Message de {_cap(s['person'])} au sujet de {s['activity']}.",
                f"La rencontre est prévue {s['date']} à {s['time']} à {s['place']}.",
                f"Le but reste simple : apprendre à {s['benefit']}.",
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"L'équipe prépare {s['place']} pour {s['activity']}.",
                f"Les participants arrivent à {s['time']} avec {s['item']}.",
                f"La date exacte est {s['date']}.",
                f"Après la séance, un court échange expliquera comment réussir à {s['benefit']}.",
            ],
            "Merci de votre aide,",
            sender,
            base_q,
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Vous avez récemment participé à {s['activity']} à {s['place']}.",
                f"Pouvez-vous dire si l'horaire de {s['time']} vous convient ?",
                f"La prochaine séance est prévue {s['date']}.",
                f"Votre avis nous aidera à mieux organiser les moments pour {s['benefit']}.",
            ],
        ...
        lambda: _email(
            ...
            "Bonjour,",
            [
                f"Nous préparons une prochaine rencontre autour de {s['activity']}.",
                f"Elle aura lieu {s['date']} à {s['time']} à {s['place']}.",
                f"Le tarif annoncé est {s['price']} et il reste {s['limit']}.",
                f"Cette fois, chacun devra apporter {s['item']}.",
            ],
            "À bientôt,",
            sender,
            base_q,
